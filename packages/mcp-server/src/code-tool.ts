...
import { ClientOptions } from '@linqapp/sdk';

const prompt = `Runs JavaScript code to interact with the Linq API V3 API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    LINQ_API_V3_API_KEY: requireValue(
      readEnv('LINQ_API_V3_API_KEY') ?? client.apiKey,
      'set LINQ_API_V3_API_KEY environment variable or provide apiKey client option',
    ),
    LINQ_API_V3_BASE_URL: readEnv('LINQ_API_V3_BASE_URL') ?? client.baseURL ?? undefined,
  };
